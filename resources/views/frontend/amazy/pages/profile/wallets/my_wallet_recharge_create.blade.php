                        ...
                        <div class="wallet_payent_box">
                            ...
                            @if (isModuleActive('Tabby') && @$payment_gateways->where('method','Tabby')->first()->active_status == 1)
                             <form action="{{ route('my-wallet.store') }}" method="post">
                                    ...
                                    <button class="wallet_elemnt" type="submit">
                                        <img class="img-fluid" src="{{showImage($gateway_activations->where('method', 'Tabby')->first()->logo)}}" alt="Tabby" title="Tabby">
                                    </button>
                             </form>
                            @endif
                            @if (isModuleActive('CCAvenue') && @$payment_gateways->where('method','CCAvenue')->first()->active_status == 1)
                                <button class="wallet_elemnt" data-bs-toggle="modal" data-bs-target="#CCAvenueModal">
                                    <img class="img-fluid" src="{{showImage($gateway_activations->where('method', 'CCAvenue')->first()->logo)}}" alt="CCAvenue" title="CCAvenue">
                                </button>
                            @endif
