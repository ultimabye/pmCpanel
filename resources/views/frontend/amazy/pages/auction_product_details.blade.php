                            ...
                            <div class="product_content_details mb_20">
                                ...
                                <div class="product_info">
                                    ...
                                    <input type="hidden" name="base_sku_price" id="base_sku_price" value="
                                        ...
                                        {{-- @if(@$product->hasDeal)
                                            {{ selling_price(@$product->skus->where('status',1)->first()->selling_price,@$product->hasDeal->discount_type,@$product->hasDeal->discount) }}
                                        @else
                                            @if($product->hasDiscount == 'yes')
                                            {{selling_price(@$product->skus->where('status',1)->first()->selling_price,@$product->discount_type,@$product->discount)}}
                                            @else
                                                {{@$product->skus->where('status',1)->first()->selling_price}}
                                            @endif
                                        @endif --}}
                                    ">
                                    ...
                                    <h5 class="mb-0">{{__('common.total')}}:
                                        <span id="total_price">
                                            {{single_price($awarded_bid->bid_amount)}}
                                            {{-- @if(@$product->hasDeal)
                                                {{single_price(selling_price(@$product->skus->where('status',1)->first()->selling_price,@$product->hasDeal->discount_type,@$product->hasDeal->discount) * $product->product->minimum_order_qty)}}
                                            @else
                                                @if($product->hasDiscount == 'yes')
                                                    {{single_price(selling_price(@$product->skus->where('status',1)->first()->selling_price,@$product->discount_type,@$product->discount) * $product->product->minimum_order_qty)}}
                                                @else
                                                    {{single_price(@$product->skus->where('status',1)->first()->selling_price * $product->product->minimum_order_qty)}}
                                                @endif
                                            @endif --}}
                                        </span>
                                    </h5>
                                ...
                                <div class="product_widget5 mb_30 style5 w-100">
                                    <div class="product_thumb_upper">
                                        ...
                                        <a href="{{singleProductURL(@$related_seller_product->seller->slug, $related_seller_product->slug)}}" class="thumb">
                                            <img src="{{$thumbnail}}" alt="{{@$related_seller_product->product_name}}" title="{{@$related_seller_product->product_name}}" class="lazyload">
                                        </a>
                                        <div class="product_action">
