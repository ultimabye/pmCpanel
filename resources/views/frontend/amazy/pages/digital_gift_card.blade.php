    ...
    <div class="container">
        <div class="row">
            ...
            <div class="col-lg-8 col-xl-9" id="dataWithPaginate">
                @include('frontend.amazy.partials._digital_giftcard_list')
            </div>
        </div>
    </div>
        ...
        (function($){
            ...
            $(document).ready(function(){
                ...
                $(document).on('change', '.filterDataChange', function(event){
                    ...
                    $('#pre-loader').show();
                    if (prev_stat == 0) {
                        var url = "{{route('frontend.digital_gift_card.fetch-data')}}";
                    }else {
                        var url = "{{route('frontend.digital_gift_card.filter_by_type')}}";
                    }
                ...
                function getProductByChoice(type, el){
                    ...
                    $('#pre-loader').show();
                    $.post('{{ route("frontend.digital_gift_card.filter_by_type") }}', {_token:'{{ csrf_token() }}', filterType:filterType}, function(data){
                        $('#dataWithPaginate').html(data);
                        $('.filterCatCol').val(1);
                        $('#product_short_list').niceSelect();
                        $('#paginate_by').niceSelect();
                ...
                function fetch_data(page){
                    ...
                    if (sort_by != null && paginate != null) {
                        var url = "{{route('frontend.digital_gift_card.fetch-data')}}"+'?sort_by='+sort_by+'&paginate='+paginate+'&page='+page;
                    }else if (sort_by == null && paginate != null) {
                        var url ="{{route('frontend.digital_gift_card.fetch-data')}}"+'?paginate='+paginate+'&page='+page;
                    }else {
                        var url = "{{route('frontend.digital_gift_card.fetch-data')}}" + '?page=' + page;
                    }
                    if(page != 'undefined'){
                        $.ajax({
                            url: url,
