...
class WalletRepository
{
    ...
    public function walletOfflineRecharge($data)
    {
        ...
        $notification = NotificationSetting::where('slug','offline-recharge')->first();
        if ($notification) {
            $this->notificationSend($notification->id, $data['user_id']);
        }
    ...
    public function walletRefundPaymentTransaction($refund_id, $refund_infos, $customer_id)
    {
        ...
        if($seller_id != 1){
            ...
            $transactionRepo->makeTransaction("Order Refund Amount", "out", "Wallet", "sales_expense", $defaultSellerAccount, "Order Refund From Sale", $refund_request, $seller_refund, Carbon::now()->format('Y-m-d'), auth()->id(), null, null);

            if($commision > 0){
                $defaultSellerCommisionAccount = $this->defaultSellerCommisionAccount();
                $transactionRepo->makeTransaction("Refund Seller Order Commision", "out", "Wallet", "refund_seller_commision", $defaultSellerCommisionAccount, "Refund Seller Order Commision", $refund_request, $commision, Carbon::now()->format('Y-m-d'), auth()->id(), null, null);
            }
        }else{
            $defaultIncomeAccount = $this->defaultIncomeAccount();
            $transactionRepo->makeTransaction("Refund from Sales", "out", "Wallet", "sales_refund", $defaultIncomeAccount, "Product Sale", $refund_request, $amount, Carbon::now()->format('Y-m-d'), auth()->id(), null, null);
        }
