...
class InstamojoController extends Controller
{
        ...
        if ($data->success == true) {
            if ($data->payment->status == "Credit") {
                ...
                if (session()->has('order_payment')) {
                    ...
                    $datas['gateway_id'] = encrypt($credential->method->id);
    ...
    private function getCredential(){
        ...
        if(session()->has('order_payment') && app('general_setting')->seller_wise_payment && session()->has('seller_for_checkout') && $is_checkout){
            $credential = getPaymentInfoViaSellerId(session()->get('seller_for_checkout'), 'instamojo');
        }else{
            $credential = getPaymentInfoViaSellerId(1, 'instamojo');
        }
        return $credential;
    }
}
