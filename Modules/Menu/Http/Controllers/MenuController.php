...
use Modules\Menu\Http\Requests\AdsRequest;
...
class MenuController extends Controller
{
    ...
    public function addColumn(Request $request){
        try{
            $menuColumn = MenuColumn::where('menu_id',$request->menu_id)->get();
    ...
    public function addElement(Request $request){
        try{
            if($request->type != 'link' && $request->type != 'function'){
    ...
    public function adsUpdate(AdsRequest $request){
        try{
            $this->menuService->adsUpdate($request->except('_token'));
            return true;
        }catch(Exception $e){
            LogActivity::errorLog($e->getMessage());
            return response()->json([
